...
await fs.ensureDir(UPLOADS_DIR); // uploadsフォルダが無ければ作成
...
app.post('/api/upload', requireLogin, upload.single('file'), (req, res) => {
  ...
  if (!file || !room) return res.status(400).json({ error: '不正なリクエストです' });
  ...
  const msgObj = {
    ...
    message: `[ファイル] ${file.originalname}`,
    fileUrl,
    originalName: file.originalname,
    timestamp: new Date().toLocaleString()
  };

  fs.readJSON(msgFile).catch(() => []).then(messages => {
    messages.push(msgObj);
    return fs.writeJSON(msgFile, messages, { spaces: 2 });
  });
